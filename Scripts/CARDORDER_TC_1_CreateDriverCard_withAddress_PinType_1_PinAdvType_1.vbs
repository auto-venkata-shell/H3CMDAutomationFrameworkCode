...
For iRowCount=1 to iTotalRows 
	...
	If ucase(strExecute) = ucase(strExecutevalue) Then
		...
		inputfilepathval = "Cardorder_withCardgroup.txt"
		outputfilepathval = "Cardorder_with_Addressoutput.txt"
		...
		cardGrpName = "CARDGROUPATEST" & randomNumber(100,999)
		'*********** Generic Code for all scripts ****************
		
		Call OpenApplication(url)
		Call navigateCustomersummaryScreen(customerERP_id)
		'Call navigateCardgroupsScreen()
		'Call navigateCardgroupOverrideScreen(cardGrpName)
		'Call validateNewFreeTextDBValidation()
		
		newcardgroupID = getCustomerCardgroupid()
		
		
		custCarddet = getCustomerValue(cardapi_data)
		custCardNo = Split(custCarddet,"|")(0)
		custColcoval = Split(custCarddet,"|")(1)
		Colcocardtypeval = Split(custCarddet,"|")(2)
		ColcoTokentypeval = Split(custCarddet,"|")(3)
		...
		'********************** Scenario specific variables and business functions *********************
		searchreplaceString = "ColCoID-" & custColcoval &";CustomerID-"& custCardNo &";CardCategoryID-" & cardapi_data("CardCatIDval") &";PINTypeID-"& cardapi_data("PinTypeIDval") &";PINAdviceType-"& cardapi_data("PinAdviceTypeidval") &";CardTypeID-" & Colcocardtypeval &";TokenTypeID-" & ColcoTokentypeval &";EmbossingID-2" &";CardGroupId-" & newcardgroupID
		...
		Else
			...
			'Call validatePostAPICardDetails()
