...
'
'''msgbox Browser("creationTime:=1").Page("creationTime:=1").WebTable("html id:=.*.*gvCardGroups").Exist
''
''Mytext = "456TEST"
''Browser("creationTime:=1").Page("creationTime:=1").Link("innertext:=Select an Option","html tag:=A").Click
''wait 2
''Browser("creationTime:=1").Page("creationTime:=1").WebElement("html id:=ctl00_CPH_multiViewSettings_ucCardMaintain_mvCard_ddlCardGroupID.*","innertext:=" & Mytext).Click
''
''wait 4
''
''
''	Set tableObj = Browser("creationTime:=1").Page("creationTime:=1").WebTable("html id:=.*gvCardGroups")
''msgbox tableObj.GetCellData(2,1)
'
'textone = Browser("name:=ntp\.msn\.com").Exist
''msgbox textone
'wait 4
...
'  		wait 2
'  		Browser("name:=ntp\.msn\.com").HandleDialog micOK
'  		wait 2
'  		Browser("creationTime:=0").HandleDialog micOK
'  		
''msgbox Browser("name:=ntp\.msn\.com").WinObject("regexpwndtitle:=Chrome Legacy Window","text:=Chrome Legacy Window").Exist
'
'msgbox Browser("name:=Select Authentication System").Page("title:=Select Authentication System").WebButton("innertext:=PingID MFA ","html tag:=BUTTON").Exist
'
'wait 2
'
'msgbox Browser("name:=Select Authentication System").Page("title:=Select Authentication System").WebButton("innertext:=Smart Card ","html tag:=BUTTON").Exist
'
'
'Browser("name:=Select Authentication System").Page("title:=Select Authentication System").WebButton("innertext:=Smart Card ","html tag:=BUTTON").Click
'wait 2
'
'
'
...
'
'
''			Browser("name:=ntp\.msn\.com").Highlight
'			wait 4
'			Set WshShell = CreateObject("WScript.Shell")
'			'Browser("name:=ntp\.msn\.com").HandleDialog micCancel
'			'  		wshshell.SendKeys strData
...
'	  		Browser("creationTime:=0").HandleDialog micOK
'	  		
'	  		Browser("creationTime:=0").HandleDialog micOK, "CA"

'
'msgbox Window("micClass:=Browser","name:=Home Page").Exist
'msgbox "Manual"
'wait 4
'msgbox Window("micClass:=Browser","name:=Home Page").Exist
