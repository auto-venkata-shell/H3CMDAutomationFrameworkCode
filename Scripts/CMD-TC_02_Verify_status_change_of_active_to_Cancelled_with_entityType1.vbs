...
For iRowCount=1 to iTotalRows 
	...
	If ucase(strExecute) = ucase(strExecutevalue) Then
		...
		'********************** Scenario specific variables and business functions *********************
		searchreplaceString = "EntityTypeId-1;ColcoCode-" & db_ColcoCode & ";StatusId-9;BlockDate-null;AccountNumber-" & newERPCustNo & ";"& "CustomerERP-" & newERPCustNo & ";"& "TopLevelCustomerNumber-" & newERPCustNo & ";"& "RegistrationNumber-Reg" & newERPCustNo & ";"& "VATRegNumber-Reg" & newERPCustNo
		...
		Else
			...
			if cint(db_ecustStatusID) = cint("9") Then
				Append_TestHTML StepCounter, "Verify Customer Status ", "Expected-9" & VBCRLF & "Actual-"& db_ecustStatusID , "PASSED"
			Else
				Append_TestHTML StepCounter, "Verify Customer Status ", "Expected-9" & VBCRLF & "Actual-"& db_ecustStatusID , "FAILED"
			End IF
		End If
	End If
next