...
For iRowCount=1 to iTotalRows 
	...
	If ucase(strExecute) = ucase(strExecutevalue) Then
		...
		'********************** Scenario specific variables and business functions *********************
		Append_TestHTML StepCounter, "Prepare INPUT JSON to change IsActive status", "Update IsActive status to False of"& topLevelERP & " and existing contact-"& externaRefVal, "PASSED"
		searchreplaceString = "EntityTypeId-7;IsActive-false;AccountNumber-" & topLevelERP & ";"& "ContactTypeId-[0];ExternalReference-" & externaRefVal & ";AddressId-" & topnewAddressref
		...
		Else
			...
			Call validateContactLines(jsonFPath,addresscustid,externaRefVal,"0","insertionupdate")
