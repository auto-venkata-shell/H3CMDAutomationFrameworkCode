...
For iRowCount=1 to iTotalRows 
	...
	If ucase(strExecute) = ucase(strExecutevalue) Then
		...
		Else
			...
			externaRefVal = "Relationship00"&lcase(countryCode) & Right(topLevelERP,6)
			...
			'********************** Scenario specific variables and business functions *********************
			searchreplaceString = "ColcoCode-" & db_ColcoCode &";EntityTypeId-3;AccountNumber-" & newERPCustNo & ";"& "ExternalReference-" & externaRefVal & ";"& "TopLevelCustomerNumber-" & topGPLevelERP & ";"& "ParentCustomerNumber-" & subPLevelERP & ";"& "PaymentCustomerNumber-" & child1LevelERP & ";"& "InvoiceCustomerNumber-" & child2LevelERP & ";"& "CardDeliveryCustomerNumber-" & child3LevelERP & ";"& "PricingCustomerNumber-" & child4LevelERP & ";"& "ContractCustomerNumber-" & child5LevelERP
			...
			Else
				...
				RelationshipLevelERP = newERPCustNo
				Call validateCustomerRelationshipData(jsonFPath,newERPCustNo)
				Call navigateToRelationshipManagementScreen(newERPCustNo)
