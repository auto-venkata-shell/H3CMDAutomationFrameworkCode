...
For iRowCount=1 to iTotalRows 
	...
	If ucase(strExecute) = ucase(strExecutevalue) Then
		...
		inputfilepathval = "BankMandatenull.txt"
		outputfilepathval = "BankMandateoutput.txt"
		...
		externaRefVal = "BankMandate02"&lcase(countryCode) & Right(topLevelERP,6)
		...
		Call basicPreConfigBankMandateData(jsonFPath,"GFN_SHELL_SPRINTQA_PH_OLTP")
		myDateEffective = nextdateDBFormt(Date()+ 10)
		...
		preSearchString = "DDEffectiveDate$" & myDateEffective & ";DDCancelDate$"& myDateTerminated
		Call searchandReplaceMultipleStringwithdollar(jsonFPath, preSearchString)
		
		'********************** Scenario specific variables and business functions *********************
		
		searchreplaceString = "EntityTypeId-9;DDMandateTypeId-1;DDActive-true;AccountNumber-" & topLevelERP & ";"& "ExternalReference-" & externaRefVal & ";BankAccountId-" & topnewBankref
		
		'Call searchandReplaceMultipleString(jsonFPath, searchreplaceString)
		Append_TestHTML StepCounter,"Set Null Values", "**** Set NULL value to DDAuthorizerName and DDReferenceNumber fields in the INPUT JSON***" ,"PASSED"
		...
		'msgbox headers
			...
			Append_TestHTML StepCounter,"Pre Customer and Bank Details", "**** Display customer-"& topLevelERP & "BankAccount and Bank Mandate Details***" ,"PASSED"
			
		Call BankMandatePrecondValues(topLevelERP,topBankAccountID,topnewBankMandateref1)
		...
		Else
			...
			Append_TestHTML StepCounter,"Validate Second Bank Mandate Date", "**** Validate all fields of M2***" ,"PASSED"
			Call validateBankMandateLines(jsonFPath,addresscustid,topBankAccountID,externaRefVal)
			topnewBankMandateref2 = externaRefVal
			Append_TestHTML StepCounter,"Validate Two Bank Mandates date differences", "**** Validate DDCancellationof M1 and DDEateEffective of M2***" ,"PASSED"
			Call validateDDCancellationdate(jsonFPath,topBankAccountID,topnewBankMandateref1,topnewBankMandateref2)
