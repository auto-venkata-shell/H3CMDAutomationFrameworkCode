...
For iRowCount=1 to iTotalRows 
	...
	If ucase(strExecute) = ucase(strExecutevalue) Then
		...
		externaRefVal = "subcontact01"&lcase(countryCode) & Right(subLevelERP,6)
		
		'*********** Generic Code for all scripts ****************
		
		myDateEffective = nextdateDBFormt(Date()-5)
		myDateTer_Year = cint(Split(myDateEffective,"-")(0)) + 5
		...
		Append_TestHTML StepCounter, "Update DateTerminated to 5 dates prior of allocated date",myDateTerminated & " - DateTerminated", "PASSED"
		'********************** Scenario specific variables and business functions *********************
		searchreplaceString = "EntityTypeId-5;AccountNumber-" & subLevelERP & ";"& "ExternalReference-" & externaRefVal & ";DateTerminated$" & myDateTerminated
		...
		Else
			...
			addresscustid = verifyPostCustomerAddressActiveDetails(subLevelERP,transReqID)
			Call validateBankLines(jsonFPath,addresscustid,externaRefVal,"update")
			subnewContactref = externaRefVal
