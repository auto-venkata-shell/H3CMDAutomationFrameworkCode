...
For iRowCount=1 to iTotalRows 
	...
	If ucase(strExecute) = ucase(strExecutevalue) Then
		...
		'********************** Scenario specific variables and business functions *********************
		Append_TestHTML StepCounter, "Prepare INPUT JSON for Update Name fields", "Appending New text to previous contacttypeid-1 values of"& topLevelERP & " and same contact-"& externaRefVal, "PASSED"
		searchreplaceString = "EntityTypeId-7;Forename-NewAutomation;MiddleName-NewTest;LastName-NewScript;AccountNumber-" & topLevelERP & ";"& "ContactTypeId-[1];ExternalReference-" & externaRefVal & ";AddressId-" & topnewAddressref
		...
		Else
			...
			Call validateContactLines(jsonFPath,addresscustid,externaRefVal,"1","update")
