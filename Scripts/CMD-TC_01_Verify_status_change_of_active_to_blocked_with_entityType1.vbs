...
For iRowCount=1 to iTotalRows 
	...
	If ucase(strExecute) = ucase(strExecutevalue) Then
		...
		'********************** Scenario specific variables and business functions *********************
		searchreplaceString = "EntityTypeId-1;ColcoCode-" & db_ColcoCode & ";StatusId-12;AccountNumber-" & newERPCustNo & ";"& "CustomerERP-" & newERPCustNo & ";"& "TopLevelCustomerNumber-" & newERPCustNo & ";"& "RegistrationNumber-Reg" & newERPCustNo & ";"& "VATRegNumber-Reg" & newERPCustNo
		...
		Else
			...
			if cint(db_ecustStatusID) = cint("12") Then
				Append_TestHTML StepCounter, "Verify Customer Status ", "Expected-12" & VBCRLF & "Actual-"& db_ecustStatusID , "PASSED"
			Else
				Append_TestHTML StepCounter, "Verify Customer Status ", "Expected-12" & VBCRLF & "Actual-"& db_ecustStatusID , "FAILED"
			End IF
		End If
	End If
next