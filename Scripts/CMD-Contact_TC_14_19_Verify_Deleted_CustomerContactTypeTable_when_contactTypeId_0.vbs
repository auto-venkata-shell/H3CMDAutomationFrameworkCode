...
For iRowCount=1 to iTotalRows 
	...
	If ucase(strExecute) = ucase(strExecutevalue) Then
		...
		'********************** Scenario specific variables and business functions *********************
		Append_TestHTML StepCounter, "Prepare INPUT JSON to delete existing contacttype entries", "Update contacttypeid-0 values of"& topLevelERP & " and existing contact-"& externaRefVal, "PASSED"
		searchreplaceString = "EntityTypeId-7;AccountNumber-" & topLevelERP & ";"& "ContactTypeId-[0];ExternalReference-" & externaRefVal & ";AddressId-" & topnewAddressref
		...
		Else
			...
			Call validateContactLines(jsonFPath,addresscustid,externaRefVal,"0","deletion")
