...
For iRowCount=1 to iTotalRows 
	...
	If ucase(strExecute) = ucase(strExecutevalue) Then
		...
		inputfilepathval = "PinReminderwithoutaddress.txt"
		outputfilepathval = "PinReminderwithoutaddressoutput.txt"
		...
		If rcheck <> Empty Then
			...
			Call verifyCardPINMailerRequestDetails(cardapi_data("PinAdviceTypeidval"),0,0)
			JobRundate = Date()
			dtsnow = Now()
			...
			if fileoutboundlog <> "" and cAddressDet <> "" Then
				cAddressDetval = Split(cAddressDet,";")(0)
				Call validateOutboundFilenegativeData(fileoutboundlog,cAddressDetval,"txt")
